...
// Constants 
final float   paper_size_x = 150; // chỉnh kích cỡ ảnh 
final float   paper_size_y = 180;
...
final float   pen_width = 0.65;               //mm, determines image_scale, reduce, if solid black areas are speckled with white holes.Độ rộng của nét vẽ của bút, tính theo mm
final int     pen_count = 1;                  //Số lượng bút mà hệ thống có thể sử dụng
final char    gcode_decimal_seperator = '.';  //để phân tách phần nguyên và phần thập phân trong các giá trị Gcode.  
...
float feetrate = 2000;
String penZup="M300 S50";
String penZdown="M300 S30";
// Every good program should have a shit pile of badly named globals.
Class cl = null;                              //có thể liên quan đến các chế độ xử lý hoặc các lớp 
pfm ocl;                                      //liên quan đến các phương pháp vẽ hoặc xử lý hình ảnh.
int current_pfm = 0;                          // Chỉ số để theo dõi chế độ vẽ hiện tại
String[] pfms = {"PFM_original", "PFM_spiral", "PFM_squares"}; //Mảng chuỗi chứa các tên của các phương pháp vẽ khác nhau (gốc, xoắn ốc, hình vuông).

int     state = 1; //  Nó có thể điều khiển các bước hoặc pha khác nhau trong quá trình vẽ hoặc xử lý ảnh.
int     pen_selected = 0;                      //  Chỉ số của bút đang được chọn 
int     current_copic_set = 0;                 //  Bộ màu COPIC hiện tại được chọn từ mảng copic_sets
int     display_line_count;                    //  Đếm số lượng đường kẻ hiển thị trong bản vẽ.
String  display_mode = "drawing";              //  Chế độ hiển thị hiện tại (ví dụ: vẽ, hiển thị lưới, hoặc các chế độ khác). 
PImage  img_orginal;               // The original image - Hình ảnh gốc được tải vào chương trình
...
float   gcode_offset_x;            // Tọa độ bù trừ trong hệ trục X và Y, được sử dụng để điều chỉnh vị trí trong Gcode.
float   gcode_offset_y;
float   gcode_scale;               //Hệ số tỷ lệ để phóng to/thu nhỏ hình ảnh khi chuyển đổi sang Gcode.
float   screen_scale;              //Hệ số tỷ lệ cho màn hình hiển thị (phóng to hoặc thu nhỏ).
float   screen_scale_org;          //Hệ số tỷ lệ cho màn hình hiển thị (phóng to hoặc thu nhỏ).
int     screen_rotate = 0;         //Góc xoay của màn hình hiển thị (theo đơn vị độ).
float   old_x = 0;                 //Tọa độ trước đó của bút, được sử dụng để theo dõi vị trí trong quá trình vẽ.
float   old_y = 0;
int     mx = 0;                    //Các tọa độ lưu vị trí của chuột hoặc điểm tham chiếu trong chương trình.
int     my = 0;
int     morgx = 0;
int     morgy = 0;
int     pen_color = 0;             //Màu hiện tại của bút.
boolean is_pen_down;                
boolean is_grid_on = false;        //Kiểm tra xem lưới có được bật hay không trong quá trình hiển thị.
String  path_selected = "";        // Đường dẫn file hiện tại được chọn.
String  file_selected = "";        // Tên file hiện tại được chọn.
String  basefile_selected = "";    // Tên cơ sở của file (dùng làm tiền tố cho các file đầu ra).
String  gcode_comments = "";       // Chuỗi lưu trữ các bình luận hoặc chú thích cho file Gcode.
int     startTime = 0;             // Thời gian bắt đầu quá trình vẽ.
boolean ctrl_down = false;         // Biến kiểm tra xem phím điều khiển (Ctrl) có đang được nhấn hay không.


Limit   dx, dy;                    //Đối tượng Limit dùng để theo dõi giới hạn tối đa và tối thiểu của các tọa độ X và Y.
Copix   copic;                     // Đối tượng Copix để lưu trữ các màu sắc và quản lý các bút vẽ COPIC.
PrintWriter OUTPUT;                //Đối tượng PrintWriter được sử dụng để ghi file (ví dụ: file Gcode).
botDrawing d1;                     //có thể đại diện cho một đối tượng vẽ hoặc bản vẽ hiện tại.

float[] pen_distribution = new float[pen_count];//Mảng pen_distribution lưu trữ tỷ lệ phân phối cho mỗi bút vẽ.

String[][] copic_sets = {                       // Mảng 2 chiều chứa các bộ màu COPIC khác nhau.
...
void setup_squiggles() {
  ...
  img_orginal = createImage(img.width, img.height, RGB);// tạo 1 ảnh trống có cùng kích thước
  img_orginal.copy(img, 0, 0, img.width, img.height, 0, 0, img.width, img.height);// copy nội dung hình ảnh của img , các thông số khác là vùng chọn copy và vùng lưu trên ảnh mới
  ...
  gcode_offset_x =0;
  gcode_offset_y =0+paper_top_to_origin;
  if((gcode_offset_y + paper_size_y * gcode_scale)>=310) gcode_offset_y = 310-paper_size_y * gcode_scale;
