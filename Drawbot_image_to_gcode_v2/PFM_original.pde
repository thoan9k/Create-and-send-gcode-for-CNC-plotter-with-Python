...
class PFM_original implements pfm {
  ...
  final int    squiggle_length = 500;      // How often to lift the pen -  sau mỗi 500 đơn vị chiều dài đường vẽ, bút sẽ được nhấc lên để chuyển sang một đường vẽ khác
  ...
  int          squiggle_count;  // Biến đếm số lượng "squiggle" đã được vẽ
  int          darkest_x; //Đây là tọa độ của điểm tối nhất (có độ sáng thấp nhất) trên hình ảnh
  int          darkest_y;
  float        darkest_value;// Độ sáng của điểm tối nhất đã tìm thấy trên hình ảnh
  float        darkest_neighbor = 256;//Biến này lưu độ sáng của vùng lân cận xung quanh điểm tối nhất. 
